...
sub _validate_body {
  ...
  if ($val->{accept} and !negotiate_content_type($param->{accepts}, $val->{accept})) {
    $val->{valid} = 0;
    return E "/header/Accept", [join(', ', @{$param->{accepts}}), type => $val->{accept}];
  }
...
C<%res> follows the same rules as C<%req> in L</validate_request>. "accept" in
body is optional, but should have the same format as the "Accept" header.
